...
**Analyse**  
+ 先考虑*至多*需要多少只鼠。显然取100只鼠足矣，*一只鼠分配一瓶药*，无脑但有效。
+ *次多*呢？想想发现99只也够了，依然是*一只鼠分配一瓶药*，最后多出来一瓶，根据另外99瓶药的检测结果，即可推断。
+ 再想，`每只老鼠完全可以喝不止一瓶药`，那么通过**构造适当的喂药方案**，应当能用更少的鼠完成任务。
+ 怎么构造呢？——接下来猜测并验证，log100向上取整得7，7只鼠即可满足要求。

喂药方案如下：  

7只鼠一字排开，从右到左依次记为`鼠1～鼠7`。100瓶药顺序摆好，记为`药瓶1～药瓶100`，依次将每瓶药选择`几只`小鼠进行投喂。那么某瓶药到底选择`哪几只`鼠进行投喂呢？这样做——计算该药瓶编号对应的二进制编码，刚好得到7个布尔值（不足7位的，在高位补0），这7个布尔值刚好对应了前述7只鼠是否喂药：值为1则喂药，值为0则不喂。比如`药瓶20`对应二进制为`0010100`，那么就只喂给`鼠3`和`鼠5`，其余不喂，因为从右向左数，只有第3、第5个位置布尔值为1。
...
以10瓶药水中1瓶有毒为例，说明问题。log10向上取整得4，就首先预备4只鼠。按照上述喂药方案，相当于对`药瓶1～药瓶10`全部进行二进制编码，并让`鼠1～鼠4`分别占据一个二进制位。有毒药水一定会致死喝了它的小鼠，反之没有喝它的小鼠则一定存活，那么根据鼠的生死情况还原出的0 1序列，刚好就是毒药瓶编号的二进制表示。整个过程就是一场编码。
